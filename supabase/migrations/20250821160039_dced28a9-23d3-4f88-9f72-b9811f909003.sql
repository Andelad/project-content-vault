...
-- Based on SUPABASE_REQUIREMENTS.md (Fixed - removed CONCURRENTLY)
...
-- Add missing triggers for existing tables (only if they don't exist)
DO $$
BEGIN
...
-- Create critical indexes for performance (without CONCURRENTLY)
...
CREATE INDEX IF NOT EXISTS idx_projects_user_group ON public.projects(user_id, group_id);
CREATE INDEX IF NOT EXISTS idx_rows_group_order ON public.rows(group_id, order_index);
CREATE INDEX IF NOT EXISTS idx_milestones_project_order ON public.milestones(project_id, order_index);
...
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'work_hours' AND table_schema = 'public') THEN
        CREATE INDEX IF NOT EXISTS idx_work_hours_user_time ON public.work_hours(user_id, start, "end");
    END IF;
END $$;