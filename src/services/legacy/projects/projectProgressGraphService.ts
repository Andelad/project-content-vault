 ...
 */
export function buildProjectPlannedTimeMap(
  events: CalendarEvent[],
  projectId: string
): Map<string, number> {
 ...
 */
export function getProjectPlannedTimeUpToDate(
...
export function generateMilestoneBasedProgressData(
  ...
  const plannedTimeMap = buildProjectPlannedTimeMap(events, project.id);
  ...
  data.push({
    ...
    plannedTime: getProjectPlannedTimeUpToDate(plannedTimeMap, startDate)
  });
  ...
  relevantMilestones.forEach((milestone) => {
    ...
    data.push({
      ...
      plannedTime: getProjectPlannedTimeUpToDate(plannedTimeMap, milestoneDate)
    });
  });
  ...
  if (!lastMilestone || new Date(lastMilestone.dueDate) < endDate) {
    ...
    data.push({
      ...
      plannedTime: getProjectPlannedTimeUpToDate(plannedTimeMap, endDate)
    });
  }
  ...
  if (options.includeEventDatePoints !== false) {
    ...
    eventDates.forEach(eventDate => {
      ...
      data.push({
        ...
        estimatedProgress,
        completedTime,
        plannedTime: getProjectPlannedTimeUpToDate(plannedTimeMap, eventDate)
      });
    });
  }
...
export function generateLinearProgressData(
  ...
  for (let i = 0; i <= samplePoints; i++) {
    ...
    data.push({
      ...
      estimatedProgress,
      completedTime,
      plannedTime: getProjectPlannedTimeUpToDate(plannedTimeMap, currentDate)
    });
  }
