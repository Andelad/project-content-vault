...
import { checkProjectOverlap, adjustProjectDatesForDrag, type ConflictDetectionResult, type DateAdjustmentResult, type Project, detectLiveDragConflicts, resolveDragConflicts } from '@/services';
...
export class TimelineDragCoordinatorService {
  ...
  static coordinateDragOperation(
    ...
    if (dragState.projectId && boundsValidation.isValid) {
      ...
      if (targetProject) {
        conflictResult = detectLiveDragConflicts(
          dragState.projectId,
          newDates,
          targetProject.rowId,
          projects
        ...
        if (conflictResult.hasConflicts) {
          adjustmentResult = resolveDragConflicts(
            newDates,
