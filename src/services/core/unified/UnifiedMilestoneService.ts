...
import * as milestoneCalculations from '../calculations/milestoneCalculations';
...
export class UnifiedMilestoneService {
  ...
  static validateBudgetAllocation(
    ...
    const totalAllocated = milestoneCalculations.calculateTotalAllocation(relevantMilestones);
    const utilization = milestoneCalculations.calculateBudgetUtilization(totalAllocated, projectBudget);
    const remaining = milestoneCalculations.calculateRemainingBudget(totalAllocated, projectBudget);
    const overage = milestoneCalculations.calculateOverageAmount(totalAllocated, projectBudget);
    ...
    if (milestones.length > 0) {
      const avgAllocation = milestoneCalculations.calculateAverageMilestoneAllocation(milestones);
      if (avgAllocation < 1) {
  ...
  static calculateMilestoneMetrics(milestones: Milestone[], projectBudget: number) {
    const totalAllocated = milestoneCalculations.calculateTotalAllocation(milestones);
    const distribution = milestoneCalculations.calculateAllocationDistribution(milestones);
