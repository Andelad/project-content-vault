...
/**
 * Timeline Column Width Constants
 * Centralized column width values to ensure consistency across all timeline components
 */
export const TIMELINE_COLUMN_WIDTHS = {
  DAYS_MODE: {
    COLUMN_WIDTH: 52,      // Width of each day column
...
export class TimelineViewport {
  ...
  static generateTimelineData(params: {
    ...
    if (mode === 'weeks') {
      ...
      let weekStart = new Date(viewportStart);
      const day = weekStart.getDay();
      const daysToSubtract = day === 0 ? 6 : day - 1;
      weekStart = addDaysToDate(weekStart, -daysToSubtract);
      // Generate array of week start dates for visible weeks only
      const dates = [];
      for (let w = 0; w < actualWeeks; w++) {
        let weekDate = new Date(weekStart);
        weekDate = addDaysToDate(weekStart, w * 7);
        weekDate = normalizeToMidnight(weekDate);
        dates.push(weekDate);
      }
      // Calculate viewport end based on the last visible week
      const lastWeekStart = dates[dates.length - 1];
      let viewportEnd = new Date(lastWeekStart);
      viewportEnd = addDaysToDate(lastWeekStart, 6);
      ...
      const dates = [];
      let actualViewportStart = new Date(viewportStart);
      actualViewportStart = normalizeToMidnight(actualViewportStart);
      for (let d = 0; d < actualDays; d++) {
        let date = new Date(actualViewportStart);
        date = addDaysToDate(actualViewportStart, d);
      ...
      let viewportEnd = new Date(actualViewportStart);
      viewportEnd = addDaysToDate(actualViewportStart, actualDays - 1);
      // Filter projects that intersect with viewport
      const filteredProjects = (projects || []).filter(project => {
        const projectStart = new Date(project.startDate);
        // For continuous projects, they're visible as long as they've started
  ...
  static calculateNavigationTarget(params: {
    ...
    if (timelineMode === 'weeks') {
      ...
      if (direction === 'next') {
        newStart = addDaysToDate(newStart, weeksToMove * 7);
      } else {
        newStart = addDaysToDate(newStart, -(weeksToMove * 7));
      }
    } else {
      ...
      newStart = new Date(currentViewportStart);
      if (direction === 'next') {
        newStart = addDaysToDate(newStart, viewportDays);
      } else {
        newStart = addDaysToDate(newStart, -viewportDays);
      }
    }
    let newEnd = new Date(newStart);
    newEnd = addDaysToDate(newStart, viewportDays - 1);
  ...
  static calculateTodayTarget(params: {
    ...
    start = addDaysToDate(start, -Math.floor(visibleDays / 2));
    start = normalizeToMidnight(start);
    let end = new Date(start);
    end = addDaysToDate(start, viewportDays - 1);
    return { start, end };
  }
  /**
  ...
  static calculateProjectScrollTarget(params: {
    ...
    if (timelineMode === 'weeks') {
      ...
      const daysToSubtract = day === 0 ? 6 : day - 1;
      start = addDaysToDate(start, -daysToSubtract);
    } else {
    ...
    let end = new Date(start);
    end = addDaysToDate(start, 30); // 30 day window
    return { start, end };
  }
  /**
  ...
  static calculateAutoScrollPosition(params: {
    ...
    if (direction === 'left') {
      newStart = addDaysToDate(newStart, -actualScrollAmount);
    } else {
      newStart = addDaysToDate(newStart, actualScrollAmount);
    }
    return newStart;
