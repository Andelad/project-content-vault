/**
 * Event-Work Hour Integration Calculations
 * 
 * Pure calculation functions for analyzing the relationship between calendar events and work hours.
 * Migrated from UnifiedEventWorkHourService (pure calculations, no business logic).
 * 
 * Responsibilities:
 * - Event-work hour overlap calculations
 * - Project time allocation analysis  
 * - Overtime and planned time calculations
 * - Work schedule generation
 * - Performance-optimized memoized calculations
 * - Cross-midnight event handling
 * 
 * NOT responsible for:
 * - Business rules/validation (goes to domain/rules/)
 * - Database operations (goes to orchestrators/)
 * - UI transformations (goes to services/ui/)
 * 
 * @module eventWorkHourIntegration
 */
