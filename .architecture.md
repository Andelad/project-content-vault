...
| **`.architecture.md`** | Project root | This file - codebase map and structure |
| **`Domain Logic.md`** | `/src/domain/` | What entities are, domain concepts |
| **`Rules Logic.md`** | `/src/domain/` | How rules work, calculations, formulas |
| **`Display Logic.md`** | `/src/domain/` | View-specific display constraints |
| **`README.md`** | `/src/components/` | Component file organization |

---

## 🏛️ Architecture Philosophy: Hybrid DDD
...
│   ├── Domain Logic.md             # What entities ARE
│   ├── Rules Logic.md              # How rules WORK
│   ├── Display Logic.md            # Where display constraints apply
│   ├── entities/                   # Rich domain models (Phase, Project, etc.)
│   │   └── Project.ts              # Example: Project entity with methods
│   └── rules/                      # Pure business rules (validation, constraints)
│       ├── ProjectRules.ts         # Project-specific validation
│       ├── PhaseRules.ts           # Phase-specific validation
│       └── RelationshipRules.ts    # Cross-entity relationships
│
├── services/                        # 🔧 Application services
│   ├── orchestrators/              # Workflows (CREATE/UPDATE/DELETE)
│   │   ├── ProjectOrchestrator.ts  # Project CRUD workflows
│   │   └── PhaseOrchestrator.ts    # Phase CRUD workflows
...
│   └── useProjectDrag.ts           # Interaction hooks
│
├── types/                           # 📝 Type definitions
│   └── core.ts                     # Central type definitions (SINGLE SOURCE)
│
└── utils/                           # 🛠️ Generic utilities
    └── dateFormatUtils.ts          # Display formatting
...
|-------|--------------|-------------------|
| **Domain Rules** | Business validation, domain logic | No UI, no DB, no presentation |
| **Domain Entities** | Rich objects with behavior | No UI concerns, no DB calls |
| **Orchestrators** | Workflows, coordination, DB access, UI prep | No business logic implementation |
| **Unified Services** | Business calculations (delegates to domain/calculations) | No DB access, no side effects |
| **Calculations** | Pure math | No business context, no DB, no UI |
| **Hooks** | React state + service coordination | No business logic |
| **Components** | Display only | No logic, no calculations |
...
### 1. Domain Logic - WHAT (Concepts)
**File:** `/src/domain/Domain Logic.md`
...
### 2. Rules Logic - HOW (Calculations)
**File:** `/src/domain/Rules Logic.md`
...
**File:** `/src/domain/Display Logic.md`
...
### Unified Services (Business Calculations)

**Purpose:** Business-aware calculations that delegate to pure math
...
- No side effects
- No database access
- Delegates to `calculations/` and `domain/rules/`

**Example Pattern:**
```typescript
class UnifiedProjectService {
  static calculateRemainingHours(project: Project): number {
    // Delegates to domain rules
    return ProjectRules.calculateRemaining(project);
  }
  
  static getDailyAllocation(project: Project): number {
    // Combines pure math + business rules
    const days = DateCalculations.calculateWorkingDays(
      project.startDate, 
      project.endDate
    );
    return project.estimatedHours / days;
  }
}
```
...
- 🚧 Client, Phase, Group, Label (planned)
...
- ✅ Entity definitions change (Domain Logic.md)
- ✅ Business rules change (Rules Logic.md)
- ✅ Calculations/formulas change (Rules Logic.md)
- ✅ View display constraints change (Display Logic.md)
- ✅ Relationships between entities change (Domain Logic.md)
...
### Update This File (.architecture.md) When:
...
**Changes:**
- Renamed `MilestoneRules` → `PhaseRules`
- Renamed `UnifiedMilestoneService` → `UnifiedPhaseService`
...
3. **Use from orchestrators/unified services**
4. **Never implement in components or hooks**

### Adding a Calculation

1. Is it pure math? → `/services/calculations/`
2. Has business context? → `/services/unified/` (delegate to calculations)
3. Entity-specific? → Domain entity method
...
| Calculate remaining hours | `unified/UnifiedProjectService.ts` | Business calculation |
...
- **Domain concepts & entities:** `/src/domain/Domain Logic.md`
- **Business rules & formulas:** `/src/domain/Rules Logic.md`
- **Display constraints:** `/src/domain/Display Logic.md`
